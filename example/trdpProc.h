...
typedef struct
{
    ...
    unsigned  short reserverd4[48];     //保留字4，预留104-199位置的字节
...
typedef struct
{
    //Byte0~49 为 HVAC 机组 1 发送给 EGWM 状态数据-----------------------------------
	unsigned char highByteOfSoftwareVersionSet1;             //本车空调机组 1 软件版本高字节
    unsigned char lowByteOfSoftwareVersionSet1;             //本车空调机组 1 软件版本低字节
    ...
    //--------------------------------------------------------------//
}HVAC_TO_EGWM_STATUSSET1;
typedef struct
{
    //Byte50~99 为 HVAC 机组 2 发送给 EGWM 状态数据-----------------------------------
	unsigned char highByteOfSoftwareVersionSet2;             //本车空调机组 2 软件版本高字节
    unsigned char lowByteOfSoftwareVersionSet2;             //本车空调机组 2 软件版本低字节
...
typedef struct
{
    ...
    //Bit7
    unsigned char faultStatus6Set2;        //机组 2 故障状态 6
    unsigned short reserverd8[22];         //保留字8，预留156-199位置的字节
}HVAC_TO_EGWM_FAULTSET2;
//-----------------------------------------------------------------------------------------------------------//
...
typedef struct
{
    ...
    //EGWM生命信号
    unsigned char lifeSign;
    
    //保留字2，预留2-99位置的字节
    unsigned short reserverd2[49];
...
{
    //EGWM生命信号高字节
    unsigned char lifeSignH;

    //EGWM生命信号低字节
    unsigned char lifeSignL;
    
    //保留字1，预留2-99位置的字节
    unsigned short reserverd1[49];
...
typedef struct
{
    ...
    //本车实际牵引/电制动力高字节（单位：0.01KN）
    unsigned char trainTractionAndBrakingForceH;

    //本车实际牵引/电制动力低字节（单位：0.01KN）
    unsigned char trainTractionAndBrakingForceL;

    //保留字2，预留4-99位置的字节
    unsigned short reserverd2[48];
...
{
    //保留字1，预留0-30位置的字节
    unsigned char reserverd1[31];

    //生命信号
    unsigned char lifeSign;

    //保留字2，预留32-199位置的字节
    unsigned short reserverd2[84];
